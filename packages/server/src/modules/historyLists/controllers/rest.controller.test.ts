...
import { historyEntryToEntity } from "../models";
import { HistoryListRestController } from "./rest.controller";
  ...
  describe("entries", () => {
    ...
    describe("getManyEntriesByHistoryListIdSearch", () => {
      ...
      it("should return all entries if empty criteria provided", async () => {
        ...
        expect(response.body).toEqual(createSuccessDataResponse(historyList.entries.map(e=>historyEntryToEntity(e, historyList))));
      } );
    } );
      ...
      it("should return all entries if no criteria provided", async () => {
        ...
        expect(response.body).toEqual(
          createSuccessDataResponse(HISTORY_LIST_SIMPSONS.entries
            .map(e=>historyEntryToEntity(e, HISTORY_LIST_SIMPSONS))),
        );
      } );
    } );
  } );
