...
export function modelToDocOdm(model: MusicQueryModel): DocOdm {
  const doc: DocOdm = {
    name: model.name,
    query: model.query,
    slug: model.slug,
...
export function docOdmToEntity(doc: FullDocOdm): MusicQueryEntity {
  const entity: MusicQueryEntity = {
    id: doc._id.toString(),
    name: doc.name,
    query: doc.query,
    slug: doc.slug,
...
export function partialModelToUpdateQuery(
  model: Partial<MusicQueryEntity>,
  ...
  if (model.imageCoverId !== undefined)
    ret.imageCoverId = model.imageCoverId ? new Types.ObjectId(model.imageCoverId) : null;
